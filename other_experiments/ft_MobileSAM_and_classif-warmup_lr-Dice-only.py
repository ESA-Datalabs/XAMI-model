...
from dataset import dataset_utils as data_set, augment as aug, astronomy_utils as astro
...
if split_dataset_here:
    ...
    ground_truth_masks, bbox_coords, classes, class_categories = data_set.get_coords_and_masks_from_json(input_dir, data) # type: ignore
    ...
    splits = data_set.split_list(raw_images, [training_size, val_size, test_size])
    training_image_paths, val_image_paths, test_image_paths = splits[0], splits[1], splits[2]
    train_gt_masks, train_bboxes = data_set.create_dataset(training_image_paths, ground_truth_masks, bbox_coords)
    test_gt_masks, test_bboxes = data_set.create_dataset(test_image_paths, ground_truth_masks, bbox_coords)
    val_gt_masks, val_bboxes = data_set.create_dataset(val_image_paths, ground_truth_masks, bbox_coords)
    ...
    train_data = data_set.load_json(json_train_path)
    valid_data = data_set.load_json(json_valid_path)
    test_data = data_set.load_json(json_test_path)
        
    train_gt_masks, train_bboxes, train_classes, train_class_categories = data_set.get_coords_and_masks_from_json(train_dir, train_data) # type: ignore
    val_gt_masks, val_bboxes, val_classes, val_class_categories = data_set.get_coords_and_masks_from_json(valid_dir, valid_data) # type: ignore
    test_gt_masks, test_bboxes, test_classes, test_class_categories = data_set.get_coords_and_masks_from_json(test_dir, test_data) # type: ignore
...
for one_path in training_image_paths[:4]:
    ...
    for name in image_masks_ids:
        predict.show_box(train_bboxes[name], plt.gca())
        predict.show_mask(maskUtils.decode(train_gt_masks[name]), plt.gca())
    plt.axis('off')
    plt.show()
    plt.close()
...
if use_augmentation: 
    ...
    for image_path in raw[:2]:
        ...
        bboxes_ = [aug.enlarge_bbox(np.array([value_i[0], math.floor(value_i[1]),  math.floor(value_i[2] - value_i[0]), \
        ...
        augmented1 = aug.augment_and_show(geometrical_augmentations, image_, masks, bboxes_, label_ids, class_categories, show_=False)
        ...
        augmented3 = aug.augment_and_show(noise_blur_augmentations, image_, masks, bboxes_, label_ids, class_categories, show_=False)
        ...
        aug.update_dataset_with_augms(augmented1, new_filename1, bbox_coords, ground_truth_masks, image_paths, classes)
        aug.update_dataset_with_augms(augmented3, new_filename3, bbox_coords, ground_truth_masks, image_paths, classes)
else:
...
# %% LR warmup and scheduler [markdown]
