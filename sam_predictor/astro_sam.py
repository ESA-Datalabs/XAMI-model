...
class AstroSAM:
    ...
    #     max_low_res_masks = torch.zeros((low_res_masks.shape[0], 1, 256, 256), device=self.device)
    #     max_ious = torch.zeros((iou_predictions.shape[0], 1), device=self.device)
    ...
    #     image_loss = (torch.mean(image_loss) + torch.mean(iou_image_loss))/2 * 1e5
    ...
    #     del threshold_masks
    #     del max_low_res_masks, max_ious, iou_predictions, low_res_masks
    #     del numpy_gt_threshold_mask 
    #     del pred_masks, gt_threshold_masks
    #     del rle_to_mask
    #     del image_masks, input_masks, input_bboxes, image_embedding, negative_mask
    #     del boxes, gt_rle_to_masks, mask_areas, gt_numpy_bboxes
    #     del transformed_losses, iou_image_loss
    #     del transformed_image, transformed_bboxes, transformed_masks
    #     torch.cuda.empty_cache()
    ...
    #         image_loss = torch.stack(image_loss)
    #         iou_image_loss = torch.stack(iou_image_loss)
    #         image_loss = torch.mean(image_loss) + torch.mean(iou_image_loss) * 1e5
    #         # if show_plot:
    #         #     for i in range(threshold_masks.shape[0]):
    #         #         fig, axs = plt.subplots(1, 3, figsize=(40, 20))
    #         #         axs[0].imshow(threshold_masks.permute(1, 0, 2, 3)[0][i].detach().cpu().numpy())
    ...
    #                 if mode == 'validate':
    #                     with torch.no_grad():
    #                             batch_loss=batch_loss+self.one_image_predict(
    #                                 image_masks, 
    #                                 gt_masks, 
    #                                 gt_bboxes, 
    #                                 image_embedding,
    ...
    #         del image_embedding, negative_mask, input_image, image, batch_loss
            
    #         self.predictor.reset_image()
    #         torch.cuda.empty_cache()
			
    #     return np.mean(losses)/1e5 # cancel the scaling factor
